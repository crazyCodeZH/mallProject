...
//跳转到菜单表格界面
  ...
  //菜单信息-列表
    ...
    //菜单列表 父菜单所有
    @ResponseBody
    @RequestMapping(value = "/treeParent",method = RequestMethod.POST)
    public void treeParent(HttpServletResponse response,PageHelper page){
        List<SysMenu> menuList = menuService.listAllParentMenu();
//        SysMenu menu = new SysMenu();
//        menu.setId(0);
//        menu.setName("无");
//        menuList.add(0,menu);
//        String json = createTreeJson(menuList);
        this.write(response,JSON.toJSONString(menuList));
    }
    //保存 新增修改
    @ResponseBody
