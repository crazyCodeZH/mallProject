...
function onAdd(){
	...
	$('#fm').form('clear');
        ...
        $('#parentidInput').combobox({  //为下拉框赋值
            url:path+"/menu/treeParent?ver="+Math.random(),
            valueField: 'id',
            loadMsg: "正在加载菜单信息，请稍等...",
            textField: 'name',
            panelWidth: 150,
            panelHeight: 'auto',
            value:"无",
            onLoadSuccess: function (data) {
             ...
             var first = data[0];
             // debugger
                $(this).combobox('setValue',first.id);
	 	...
	 	$('#fm').form('load',row);//这句话有问题，第一次加载时正确的，第二次就出错了，还保持第一次的数据
          ...
          $('#parentidInput').combobox({  //为下拉框赋值
              ...
              onLoadSuccess: function (data) {
                  ...
                  $(this).combobox('setValue',row.parentid);
...
function onSave(){
 	$('#fm').form('submit',{
		...
		success: function(result){
	 		debugger
	 		console.log("进入成功方法==%@",result.toString());
